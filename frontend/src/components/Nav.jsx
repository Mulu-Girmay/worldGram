...
const Nav = () => {
  ...
  const orderedStories = React.useMemo(() => {
    ...
    const ownStories = (stories || []).filter(
      (story) =>
        String(story?.authorId?._id || story?.authorId || "") === ownId,
    );
    ...
    <div className="space-y-3">
      <div className="flex items-center gap-2 rounded-2xl border border-[var(--border-color)] bg-[var(--surface-color)] px-3 py-3 shadow-sm">
        <button
          ...
          className="grid h-9 w-9 place-items-center rounded-xl border border-[var(--border-color)] bg-white p-0 text-[#2f5b2f] transition-all duration-200 hover:-translate-y-0.5 hover:scale-105 hover:shadow-sm active:scale-95"
        ...
        <div
          className="flex min-w-0 flex-1 items-center gap-2 overflow-x-auto scroll-smooth"
          onWheel={(e) => {
        ...
        >
          <button
            ...
            className="grid h-8 w-8 shrink-0 place-items-center rounded-full border border-[#6fa63a]/30 bg-white text-[#2f5b2f] transition-all duration-200 hover:-translate-y-0.5 hover:scale-105 hover:shadow-sm active:scale-95"
          ...
          <button
            ...
            className="grid h-10 w-10 shrink-0 place-items-center rounded-full border-2 border-dashed border-[#6fa63a]/70 bg-[#eaf4e2] text-[#2f5b2f] transition-all duration-200 hover:-translate-y-0.5 hover:scale-105 hover:shadow-sm active:scale-95"
                    ...
                    onClick={() =>
                      navigate("/story", {
                        state: {
                          storyId: story?._id,
                          authorId:
                            story?.authorId?._id || story?.authorId || null,
                        },
                      })
                    }
                    className={`h-10 w-10 shrink-0 overflow-hidden rounded-full border-2 ${
                      isViewed ? "border-gray-400/70" : "border-[#4c8bf5]"
                    } bg-[#eaf4e2] transition-all duration-200 hover:-translate-y-0.5 hover:scale-105 hover:shadow-sm active:scale-95`}
